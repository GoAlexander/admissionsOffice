	...
	public static void writeListOfSubmittedDocuments() throws Exception {
		...
		for (int s_i = 0; s_i < specialities.length; s_i++) {
			...
			if (countAllAbitsOnCurSpec > 0) {
				...
				row = sheet.createRow(rowNum++);
				row.createCell(1).setCellValue((moduleType.equals("аспирантура") ? "Направление: " : "Специальность: ") + specialities[s_i][1]);
				...
				for (int cg_i = 0; cg_i < competitiveGroups.length; cg_i++) {
					...
					if (competitiveGroups[cg_i][0].equals("1")) {
						...
						query = moduleType.equals("аспирантура") 
								...
								: "select * from AbiturientCompetitiveGroup where speciality = '" + specialities[s_i][0] + "' and targetOrganisation is not null";
						cstmt = con.prepareCall(query, 1004, 1007);
						rset = cstmt.executeQuery();
						...
						for (int es_i = 0; es_i < educationStandarts.length; es_i++) {
							...
							if (countAbitsOnCurSpecOnCurCompGrAndSt > 0) {
								...
								if (moduleType.equals("аспирантура")) {
									...
									row.createCell(1).setCellValue("по общему конкурсу" + (competitiveGroups[cg_i][0].equals("3") ? " (для завершивших обучение по " + educationStandarts[es_i][1] + ")" : ""));
								row.getCell(1).setCellStyle(styleForTargetOrgs);
								rowNum++;
		...
		String path = currentPath + "\\files\\" + "Список_подавших_" + moduleType + "_" + (new SimpleDateFormat("dd.MM.yyyy").format(new Date())) + ".xls";
		File file = new File(path);
		if (file.exists())
			file.delete();
		file.createNewFile();
